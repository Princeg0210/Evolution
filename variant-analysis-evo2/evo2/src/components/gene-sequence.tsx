  ...
  return (
    <Card className="biotech-card border-none">
      ...
      <CardContent className="pb-4">
        {geneBounds && (
          <div className="mb-4 flex flex-col">
            <div className="mb-2 flex flex-col items-center justify-between text-xs sm:flex-row">
              <span className="flex items-center gap-1 text-[#0A2818]/70">
                <p className="sm:hidden">From: </p>
                <p>
                  {Math.min(geneBounds.min, geneBounds.max).toLocaleString()}
                </p>
              </span>
              <span className="text-[#0A2818]/70">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                >
                  {/* Track background */}
                  <div className="absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full bg-[#F4FBF8]"></div>

                  {/* Selected range */}
                  <div
                    className="absolute top-1/2 h-2 -translate-y-1/2 cursor-grab rounded-full bg-[#0A2818] active:cursor-grabbing"
                    style={{
                  ...
                  {/* Start handle */}
                  <div
                    className="absolute top-1/2 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full border-2 border-[#0A2818] bg-white shadow active:cursor-grabbing"
                    style={{ left: `${sliderValues.start}%` }}
                    onMouseDown={(e) => handleMouseDown(e, "start")}
                  >
                    <div className="h-3 w-1 rounded-full bg-[#0A2818]"></div>
                  </div>
              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[#0A2818]/70">Start:</span>
                  <Input
                    value={startPosition}
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    className="h-7 w-full border-[#0A2818]/10 text-xs sm:w-28"
                  />
                </div>
                <Button
                  size="sm"
                  ...
                  className="h-7 w-full cursor-pointer bg-[#059669] text-xs text-white hover:bg-[#047857] sm:w-auto"
                >
                  {isLoading ? <DNALoader size="sm" /> : "Load sequence"}
                </Button>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[#0A2818]/70">End:</span>
                  <Input
                    value={endPosition}
        ...
        <div className="w-full rounded-md bg-[#F4FBF8]/50 p-3">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2">
              <DNALoader />
              <span className="text-[10px] text-[#5F8272]">Sequencing DNA…</span>
            </div>
          ) : sequenceData ? (
            ...
            <p className="text-center text-sm text-[#0A2818]/60">
        ...
        {hoverPosition !== null && mousePosition !== null && (
          <div
            className="pointer-events-none fixed z-50 rounded bg-[#0A2818] px-2 py-1 text-xs text-white shadow-md"
            style={{
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-[#0A2818]/70">A</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-[#0891B2]"></div>
            <span className="text-xs text-[#0A2818]/70">T</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-green-600"></div>
            <span className="text-xs text-[#0A2818]/70">G</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-amber-600"></div>
            <span className="text-xs text-[#0A2818]/70">C</span>
          </div>
        </div>
      </CardContent>
    </Card>
