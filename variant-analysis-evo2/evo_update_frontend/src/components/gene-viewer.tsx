...
export default function GeneViewer({
  ...
  const [geneAnalysis, setGeneAnalysis] = useState<GeneAnalysisSummary | null>(
    null,
  );
    ...
    const initializeGeneData = async () => {
      ...
      try {
        const {
          ...
          analysis,
        } = await fetchGeneAnalysis(gene, genomeId);

        setGeneDetail(fetchedDetail);
        setGeneBounds(fetchedGeneBounds);
        setGeneAnalysis(analysis);
  ...
  }, [fetchGeneSequence, gene, genomeId]);
