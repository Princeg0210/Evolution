...
export default function KnownVariants({
  ...
  const analyzeVariant = async (variant: ClinvarVariant) => {
    const rawPosition = variant.location
      ...
      : NaN;
    const codingRegex = /c\.\d+([ATGC])>([ATGC])/i;
    const fallbackRegex = /([ATGC])>([ATGC])/i;

    const refAltMatch =
      codingRegex.exec(variant.title) ?? fallbackRegex.exec(variant.title);
    ...
    if (!ref || !alt || isNaN(rawPosition)) {
      ...
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
        detailError: "Cannot parse variant position/allele from title",
      });
      return;
    }
    ...
    try {
      ...
      const analysis = await fetchClinvarVariantAnalysis(
        variant.clinvar_id,
        gene,
      );
      ...
      if (data && analysis) {
        ...
        updateClinvarVariant(variant.clinvar_id, {
          ...variant,
          isAnalyzing: false,
          detailError: "No matching record found in ClinVar",
        });
      }
    } catch (error) {
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
        detailError: error instanceof Error ? error.message : "Fetch failed",
      });
    }
  };
